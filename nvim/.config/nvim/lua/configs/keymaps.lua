...
-- 通用只读浮窗（ft 可选，如 "markdown" 会用 Snacks 渲染器渲染）
local function show_float(title, lines, ft)
	local width = 0
	...
	local w = math.min(width + 2, max_w)
	...
	vim.api.nvim_open_win(buf, true, {
		...
		width = w,
		height = h,
		row = math.max(1, math.floor((vim.o.lines - h) / 2)),
	...
	if ft == "markdown" then
		pcall(function()
			require("markview").strict_render:render(buf)
		end)
	end
end

-- mini.ai 常用快捷键速查（浮动窗口）
local function mini_ai_help()
	show_float("mini.ai", {
		"mini.ai 常用快捷键  (前缀 a=around / i=inside)",
		"配合操作符：c 改 / d 删 / y 复制，或可视模式 v",
		"",
		"改：  ciw 当前词   ci( ci{ ci\" 括号引号内   cia 参数   cif 函数调用   cit 标签",
		"      cin 下一个对象   cil 上一个对象",
		"删：  diw   di(   da\"   ...",
		"复制：yiw   yi(   ...",
		"可视：vi(   va\"   vit   via   vif   vin   vil",
		"移动：g[ 跳到 a 对象左边缘    g] 跳到右边缘",
	})
end

-- KEYBINDINGS.md 分类速查（Snacks.picker，左列表右预览）
local function keybindings_help()
	local path = vim.fn.stdpath("config") .. "/KEYBINDINGS.md"
	local ok, lines = pcall(vim.fn.readfile, path)
	if not ok or type(lines) ~= "table" or #lines == 0 then
		vim.notify("未找到 KEYBINDINGS.md", vim.log.levels.WARN)
		return
	end
	...
	if #sections == 0 then
		vim.notify("KEYBINDINGS.md 里没有分类", vim.log.levels.WARN)
		return
	end
	local items = {}
	for i, s in ipairs(sections) do
		items[i] = { text = s.title, title = s.title, lines = s.lines }
	end
	require("snacks").picker.pick({
		title = "快捷键速查 (KEYBINDINGS.md)",
		items = items,
		format = "text",
		preview = function(ctx)
			ctx.preview:reset()
			ctx.preview:set_title(ctx.item.title)
			ctx.preview:set_lines(ctx.item.lines)
			ctx.preview:highlight({ ft = "markdown" })
		end,
		confirm = function(picker, item)
			picker:close()
			vim.schedule(function()
				show_float(item.title, item.lines, "markdown")
			end)
		end,
	})
end
map("n", "<leader>km", mini_ai_help, { desc = "mini.ai 常用快捷键" })
map("n", "<leader>kk", keybindings_help, { desc = "全部快捷键速查" })
...
wk.add({
	...
	-- 包围符号 (nvim-surround，避开 Flash 的 s/S)
	{ "gz", group = "包围符号 (surround)" },
	{ "gza", desc = "加包围 gza{motion}{符号}" },
	{ "gzc", desc = "改包围 gzc{目标}{替换}" },
	{ "gzd", desc = "删包围 gzd{符号}" },

	-- 文件编辑 (nvim-surround 的 <leader> 别名)
	{ "<leader>z", group = "文件编辑", icon = "󰏫 " },
	{ "<leader>za", desc = "加包围 <leader>za{motion}{符号}" },
	{ "<leader>zc", desc = "改包围 <leader>zc{目标}{替换}" },
	{ "<leader>zd", desc = "删包围 <leader>zd{符号}" },

	-- 速查表
	{ "<leader>k", group = "速查表", icon = "󰋽 " },
	{ "<leader>km", desc = "mini.ai 常用快捷键" },
	{ "<leader>kk", desc = "全部快捷键速查（按分类）" },
})
