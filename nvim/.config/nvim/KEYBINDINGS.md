...
| `<leader>km` | n | mini.ai 常用快捷键速查（浮动窗口，速查表组） |
| `<leader>kk` | n | 全部快捷键速查（按分类，Snacks.picker，速查表组） |

## 十三、文件编辑（nvim-surround）

按空格 →「文件编辑」组，或直接使用下列键位。包围操作有两套入口（`gza…` 与 `<leader>za…`），功能相同。

| 按键 | 模式 | 功能 |
|---|---|---|
| `gza{motion}{符号}` / `<leader>za{motion}{符号}` | n | 加包围，如 `gzaiw"` 给当前词加双引号 |
| `gzc{目标}{替换}` / `<leader>zc{目标}{替换}` | n | 改包围，如 `gzc"'` 双引号改单引号 |
| `gzd{符号}` / `<leader>zd{符号}` | n | 删包围，如 `gzd"` |
| `gza` / `<leader>za` | x | 给可视选区加包围 |
| `<C-g>s` / `<C-g>S` | i | 插入模式原地 / 换行加包围 |

符号：`(`/`)` 圆括号（开括号带空格）、`{`/`}`、`[`/`]`、`<`/`>`、`'`/`"`/`` ` `` 引号、`t` HTML 标签、`f` 函数调用。

## 十四、补全（blink.cmp，插入模式内）
