...
SCAN_DIRS = [
    ...
    os.path.expanduser("~/.local/share/flatpak/exports/share/applications") # Flatpak 用户应用
]
...
def process_file(src_dir, filename):
    ...
    for line in lines:
        ...
        if is_main_entry:
            if line.startswith("Name[zh_CN]=") or line.startswith("GenericName[zh_CN]=") or line.startswith("Keywords[zh_CN]="):
    ...
    print(f"🔥 完美适配: {filename} -> 提取到中文: {list(set(found_chinese_words))} 拼音标签: {pinyin_tags}")
    return True
