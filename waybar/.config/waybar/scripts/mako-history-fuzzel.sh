#!/bin/bash
# Filename：mako-history-fuzzel.sh
# Author：lizhengbei
...
# sort_by(.id) 为升序，加上 reverse 变为降序（最新收到的通知排在 fuzzel 最顶端）
FORMATTED_LIST=$(makoctl history -j | jq -r 'sort_by(.id) | reverse | .[] | "\(.id): [\(.app_name)] \(.summary) - \(.body)"')

# 检查历史是否为空
if [ -z "$FORMATTED_LIST" ]; then
    echo "当前没有历史通知" | fuzzel --dmenu --prompt "历史通知: " --width 40 --lines 1
    exit 0
fi

# 2. 将排序后的列表送入 fuzzel 菜单
# 注：fuzzel 的 --width 含义是字符数（columns），而非 wofi 的像素值（pixels）
CHOICE=$(echo "$FORMATTED_LIST" | fuzzel --dmenu --prompt "历史通知: " --width 40 --lines 15)

# 3. 如果用户在 fuzzel 中选中了某条历史，则提取其信息并在屏幕上重新弹窗复现
if [ -n "$CHOICE" ]; then
    # 提取被选中通知的原始 ID
