...
# Description：高颜值全功能控制条版独立媒体控制器（原生 MPD 六键联动、焦点自适应关闭、微调进度条联动）
...
class MediaControlCenter(Gtk.Window):
    def __init__(self):
        ...
        # 标记用户是否正在拖动进度条，防止拖动时被后台定时刷新强行打断复位
        ...
        # ─── ➕ 新增：优雅级控制进度条胶囊舱 ───
        progress_hbox = Gtk.Box(orientation=Gtk.Orientation.HORIZONTAL, spacing=0)
        progress_hbox.set_halign(Gtk.Align.FILL)
        progress_hbox.get_style_context().add_class("progress-capsule")
        right_vbox.pack_start(progress_hbox, False, False, 2)

        # 使用 Gtk.Scale 作为进度条，范围 0-100 表示百分比
        self.progress_slider = Gtk.Scale.new_with_range(
            Gtk.Orientation.HORIZONTAL, 0, 100, 1
        )
        self.progress_slider.set_draw_value(False)  # 隐藏原生数字提示
        self.progress_slider.set_hexpand(True)
        self.progress_slider.get_style_context().add_class("media-slider")

        # 绑定进度条事件，实现平滑点击与拖拽变更进度
        self.progress_slider.connect("button-press-event", self.on_slider_pressed)
        self.progress_slider.connect("button-release-event", self.on_slider_released)
        progress_hbox.pack_start(self.progress_slider, True, True, 0)
    ...
    def apply_css(self):
        ...
        .media-slider highlight {
            background-color: #EBCB8B; /* 提取自你的play-btn主打色 */
            border-radius: 3px;
        }
        .media-slider slider {
    ...
    def on_slider_released(self, widget, event):
        # 抬起鼠标时，根据滑块当前值直接切歌进度
        val = int(self.progress_slider.get_value())
        self.run_mpc(["seek", f"{val}%"])
        self.is_seeking = False

    def mpd_status_loop(self):
        # 正则表达式用于精准提取类似 "70%" 或 "0%" 的进度数据
        ...
        while self.is_running:
            ...
            if output:
                ...
                if len(lines) >= 2 and (
                    ...
                    # ─── ➕ 从 mpc 输出行提取百分比进度 ───
                    match = progress_pattern.search(lines[1])
                    if match:
            ...
            GLib.idle_add(
                self.refresh_ui, title, artist, status_text, current_file, percentage
            )
            time.sleep(0.4)

    def refresh_ui(self, title, artist, status_text, current_file, percentage):
        ...
        if not self.is_seeking:
            self.progress_slider.set_value(percentage)
